...
def create_solution_images(text: str) -> list[io.BytesIO]:
    ...
    # Жесткий путь к системному шрифту DejaVu Sans, который мы только что установили через apt-get
    font_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
    font_bold_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"

    # Загружаем шрифт с гарантией (если файл не найдет, код упадет и покажет ошибку в логах, а не нарисует квадратики)
    font = ImageFont.truetype(font_path, 20)
    title_font = ImageFont.truetype(font_bold_path, 24)
    print("Шрифт успешно загружен из системы!")
...
def create_solution_images(text: str) -> list[io.BytesIO]:
    ...
    font_bold_path = "/app/arialbd.ttf"

    try:
        font = ImageFont.truetype(font_path, 20)
        title_font = ImageFont.truetype(font_bold_path, 24)
    except Exception as e:
        print(f"Не удалось загрузить Arial, используем дефолт: {e}")
